...
impl Graph{
    ...
    */ 
    pub fn random_distr(input : Input) -> Vec<u32> 
    {
        ...
        // for i in 0..m_e{
        //     let sigma : f64 = m_e as f64 / 10.0; 
        //     let mu : f64 = m_e as f64 / 2.0;
        //     let exp : f64 = -0.5*((i as f64 - mu)/sigma).powf(2.0); 
        //     let c : f64 = 0.398942280401432677f64; 
        //     //1/((sigma)*sqrt(2*PI)) * pow(e,pow(-0.5(i as 64-mu)/sigma, 2))
        //     println!("{}", c*(1.0/sigma)*E.powf(exp)); 
        // }

        while e > 0{
            //choose the index of the edge_number 
            let range = Uniform::from(0..edge_nums.len());
            let i = range.sample(&mut thread_rng()); 
            
            if edge_vec[edge_nums[i as usize] as usize] != 1 {
                let y: f64 = rng.gen();
                let cond = match input.dist {
                    Some(DistKind::uniform) => (input.edges as f64)/(m_e as f64),  
                    Some(DistKind::skewed) => {
                        let h : f64 = (2.0 * input.edges as f64)/(m_e as f64); 
                        let m : f64 = - h / (m_e as f64); 
                        
                        h + (i as f64)*m
                    }
                    Some(DistKind::normal) => {
                        let sigma : f64 = m_e as f64 / 10.0; 
                        let mu : f64 = m_e as f64 / 2.0;
                        let exp : f64 = -0.5*((i as f64 - mu)/sigma).powf(2.0); 
                        let c : f64 = 0.398942280401432677f64; 
                        //1/((sigma)*sqrt(2*PI)) * pow(e,pow(-0.5(i as 64-mu)/sigma, 2))
                        c*(1.0/sigma)*E.powf(exp) 
                    }
                    None => 0.0, 
                }; 
                if y < cond 
                {
                    edge_vec[edge_nums[i as usize] as usize] = 1;
                    e -=1;  
                }
            }
            //After you chose one potential edge, remove it until all edges have been chosen  
            Some(edge_nums.swap_remove(i as usize)); 
